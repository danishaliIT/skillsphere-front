...
import { 
  User, BookOpen, GraduationCap, Briefcase, 
  Award, CreditCard, Bell, LogOut, Search, 
  LayoutDashboard, Menu, X 
} from 'lucide-react';
import API from '../api/axios';

// Backend ka base address define karein
...
const StudentLayout = () => {
  ...
  // Helper logic: Relative path ko full URL mein badalne ke liye
  ...
  const navItems = [
    ...
    { name: 'Browse Courses', path: '/dashboard/browse-courses', icon: Search },
    { name: 'Enrolled Courses', path: '/dashboard/enrolled-courses', icon: BookOpen },
    { name: 'Go Training', path: '/dashboard/active-trainings', icon: GraduationCap },
    { name: 'My Enrollments', path: '/dashboard/my-trainings', icon: Briefcase },
    ...
    <div className="min-h-screen bg-[#fcfcfd] flex">
      ...
      `}>
        <div className="flex items-center gap-3 mb-12">
          <div className="size-10 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black italic shadow-xl shadow-blue-100">D</div>
          <span className="text-2xl font-black tracking-tighter italic text-gray-900 uppercase">DragonTech</span>
        </div>
      ...
      <main className="flex-1 lg:ml-72 flex flex-col min-h-screen">
        ...
        <header className="h-24 px-8 md:px-12 flex items-center justify-between border-b border-gray-50 bg-white/50 backdrop-blur-md sticky top-0 z-30">
          <div className="hidden md:block">
            <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] italic">Student Panel</h2>
          </div>
          
          <div className="flex items-center gap-4 ml-auto">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-black text-gray-900 italic leading-none">
                {profile?.first_name || 'Danish'} {profile?.last_name || 'Ali'}
              </p>
              <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mt-1">Active Student</p>
            </div>
            ...
            <div className="size-12 rounded-2xl overflow-hidden border-2 border-white shadow-lg shadow-gray-200 bg-gray-50 flex items-center justify-center">
              <img 
                src={getImageUrl(profile?.profile_picture) || `https://ui-avatars.com/api/?name=${profile?.first_name || 'D'}+${profile?.last_name || 'A'}&background=0D8ABC&color=fff`} 
                alt="profile" 
                className="w-full h-full object-cover"
                onError={(e) => { e.target.src = 'https://ui-avatars.com/api/?name=User'; }} // Fallback if image fails
              />
            </div>
          </div>
        </header>
